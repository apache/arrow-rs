...
impl IpcDataGenerator {
    ...
    /// padding).
    ///
    /// The message header is located in `ipc_write_context`'s [`FlatBufferBuilder`]
    /// finished bytes. A successful Result from this function guarantees the builder
    ...
    fn dictionary_batch_to_bytes(
        ...
        Ok(EncodedData {
            ipc_message: compression_context.mut_fbb().finished_data().to_vec(),
            arrow_data,
        })
    }

    /// Encodes a dictionary batch's flatbuffer metadata into `compression_context`'s
    /// builder (read back via `compression_context.mut_fbb().finished_data()`) and fills
    ...
    fn dictionary_batch_to_sink(
        ...
        // Reuse the builder's internal buffer across messages; `reset` keeps the
        // allocated capacity and only clears the in-progress state.
        compression_context.mut_fbb().reset();
...
pub fn write_message<W: Write>(
    ...
    let body_len = if arrow_data_len > 0 {
        write_body_buffer(&mut writer, &encoded.arrow_data, write_options.alignment)?
    } else {
        0
    };
