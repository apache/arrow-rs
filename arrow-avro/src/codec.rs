...
impl AvroField {
    ...
    ///
    /// # Returns
    /// A new `AvroField` with the same structure, but with string types
...
fn make_data_type<'a>(
    ...
    match schema {
        ...
        Schema::Complex(c) => match c {
            ...
            ComplexType::Map(m) => {
                let mut field =
                    make_data_type(m.values.as_ref(), namespace, resolver, use_utf8view)?;

                // https://avro.apache.org/docs/1.11.1/specification/#logical-types
                match (m.attributes.logical_type, &mut field.codec) {
                ...
                if !m.attributes.additional.is_empty() {
                    for (k, v) in &m.attributes.additional {
