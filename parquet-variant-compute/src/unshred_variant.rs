...
use arrow::array::{
    Array, ArrayRef, AsArray as _, BinaryViewArray, BooleanArray, FixedSizeBinaryArray,
    NullBufferBuilder, PrimitiveArray, StringArray, StructArray,
};
use arrow::datatypes::{
...
pub fn unshred_variant(array: &VariantArray) -> Result<VariantArray> {
    ...
    // Top-level context: None/None case -> create null builder
    let metadata = array.metadata_field();
    let mut row_builder =
        make_unshred_variant_row_builder(metadata, array.value_field(), array.typed_value_field())?
            .unwrap_or_else(|| {
                UnshredVariantRowBuilder::Null(NullUnshredVariantBuilder::new(array))
            });
    ...
    for i in 0..array.len() {
        if array.is_null(i) {
            ...
            let mut value_builder = value_builder.as_builder_ext(metadata);
            row_builder.append_row(&mut value_builder, i)?;
...
impl<'a> UnshredVariantRowBuilder<'a> {
    ...
    fn append_row(&mut self, builder: &mut impl VariantBuilderExt, index: usize) -> Result<()> {
        match self {
            Self::PrimitiveInt8(b) => b.append_row(builder, index),
            Self::PrimitiveInt16(b) => b.append_row(builder, index),
            Self::PrimitiveInt32(b) => b.append_row(builder, index),
            Self::PrimitiveInt64(b) => b.append_row(builder, index),
            ...
            Self::PrimitiveString(b) => b.append_row(builder, index),
            Self::PrimitiveBinaryView(b) => b.append_row(builder, index),
            Self::PrimitiveUuid(b) => b.append_row(builder, index),
            Self::Struct(b) => b.append_row(builder, index),
            Self::ValueOnly(b) => b.append_row(builder, index),
            Self::Null(b) => b.append_row(builder, index),
        }
    }
}
...
fn make_unshred_variant_row_builder<'a>(
    ...
    let Some(typed_value) = typed_value else {
        ...
        return Ok(value.map(|v| {
            UnshredVariantRowBuilder::ValueOnly(ValueOnlyUnshredVariantBuilder::new(metadata, v))
        }));
    };
    ...
    let builder = match typed_value.data_type() {
        ...
        DataType::Struct(_) => UnshredVariantRowBuilder::Struct(StructUnshredVariantBuilder::new(
            metadata,
            value,
...
impl<'a> NullUnshredVariantBuilder<'a> {
    fn new(array: &'a VariantArray) -> Self {
        Self { array }
    }
...
struct ValueOnlyUnshredVariantBuilder<'a> {
    metadata: &'a arrow::array::BinaryViewArray,
    value: &'a arrow::array::BinaryViewArray,
}

impl<'a> ValueOnlyUnshredVariantBuilder<'a> {
    fn new(metadata: &'a BinaryViewArray, value: &'a BinaryViewArray) -> Self {
        Self { metadata, value }
    }

    fn append_row(&mut self, builder: &mut impl VariantBuilderExt, index: usize) -> Result<()> {
        if self.value.is_null(index) {
            builder.append_null(); // Context-aware null handling
        } else {
            let variant = Variant::new(self.metadata.value(index), self.value.value(index));
...
macro_rules! handle_unshredded_case {
    ($self:expr, $builder:expr, $index:expr, $partial_shredding:expr) => {{
        let value = $self.value.as_ref().filter(|v| v.is_valid($index));
        let value = value.map(|v| Variant::new($self.metadata.value($index), v.value($index)));
...
impl<'a, T: Array + PrimitiveVariantUnshredExt> UnshredPrimitiveRowBuilder<'a, T> {
    ...
    fn append_row(&mut self, builder: &mut impl VariantBuilderExt, index: usize) -> Result<()> {
        handle_unshredded_case!(self, builder, index, false);
...
impl TimestampType for TimestampMicrosecondType {
    fn to_datetime_utc(micros: i64) -> Result<DateTime<Utc>> {
        DateTime::from_timestamp_micros(micros).ok_or_else(|| {
            ArrowError::InvalidArgumentError(format!(
                "Invalid timestamp microsecond value: {}",
                micros
            ))
        })
    }
...
impl<'a> StructUnshredVariantBuilder<'a> {
    fn new(
        metadata: &'a BinaryViewArray,
        value: Option<&'a BinaryViewArray>,
        typed_value: &'a StructArray,
    ) -> Result<Self> {
        // Create unshredders for each field in constructor
        let mut field_unshredders = IndexMap::new();
        for (field_name, field_array) in typed_value
            .column_names()
            .iter()
            .zip(typed_value.columns().iter())
        {
            // Factory returns None for None/None case -- these are missing fields we should skip
            let (value, typed_value) = ShreddingState::extract_fields_from(field_array.as_struct());
            let field_unshredder = make_unshred_variant_row_builder(metadata, value, typed_value)?;
            field_unshredders.insert(*field_name, field_unshredder);
        }
    ...
    fn append_row(&mut self, builder: &mut impl VariantBuilderExt, index: usize) -> Result<()> {
        let value = handle_unshredded_case!(self, builder, index, true);
        ...
        for (field_name, field_unshredder_opt) in &mut self.field_unshredders {
            if let Some(field_unshredder) = field_unshredder_opt {
                ...
                field_unshredder.append_row(&mut field_builder, index)?;
            }
        }
