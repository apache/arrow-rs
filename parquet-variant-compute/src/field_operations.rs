...
use crate::variant_parser::{VariantParser, ObjectHeader, ObjectOffsets};
...
impl VariantPath {
    ...
    pub fn push_field(mut self, name: &str) -> Self {
        self.elements.push(VariantPathElement::Field(name.to_string()));
        self
    }
...
impl FieldOperations {
    ...
    pub fn extract_field_bytes(
        ...
        for i in 0..num_elements {
            ...
            let field_id = VariantParser::unpack_int(&value_bytes[field_id_offset..], header.field_id_size)?;
            
            if field_id == target_field_id {
                return Self::extract_field_value_at_index(value_bytes, &header, &offsets, i, num_elements);
            }
        }
    ...
    fn find_field_ids(metadata_bytes: &[u8], field_names: &[&str]) -> Result<HashSet<usize>, ArrowError> {
        let metadata = VariantMetadata::try_new(metadata_bytes)?;
    ...
    fn extract_field_value_at_index(
        ...
        for i in 0..=num_elements {
            ...
            let offset_val = VariantParser::unpack_int(&value_bytes[offset_idx..], header.field_offset_size)?;
    ...
    fn collect_fields_to_keep(
        ...
        for i in 0..num_elements {
            ...
            if !target_field_ids.contains(&field_id) {
                if let Some(field_value) = Self::extract_field_value_at_index(value_bytes, header, offsets, i, num_elements)? {
    ...
    fn reconstruct_object(fields: Vec<(usize, Vec<u8>)>) -> Result<Option<Vec<u8>>, ArrowError> {
        ...
        // Write header
        let new_header = VariantParser::build_object_header(new_is_large, new_field_id_size, new_field_offset_size);
        ...
        for element in path.elements() {
            match element {
                VariantPathElement::Field(field_name) => {
                    if let Some(field_bytes) = Self::get_field_bytes(metadata_bytes, &current_value, field_name)? {
                ...
                VariantPathElement::Index(idx) => {
                    if let Some(element_bytes) = Self::get_array_element_bytes(metadata_bytes, &current_value, *idx)? {
...
mod tests {
    ...
    fn test_extract_field_bytes() {
        ...
        let nonexistent_bytes = FieldOperations::extract_field_bytes(&metadata, &value, "nonexistent").unwrap();
    ...
    fn test_remove_field_bytes() {
        ...
        let city_bytes = FieldOperations::extract_field_bytes(&metadata, &new_value, "city").unwrap();
        assert!(city_bytes.is_none());
        
        // Verify other fields are still there
        let name_bytes = FieldOperations::extract_field_bytes(&metadata, &new_value, "name").unwrap();
        assert!(name_bytes.is_some());
    }
} 