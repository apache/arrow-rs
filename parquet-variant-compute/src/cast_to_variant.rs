...
use arrow::array::{
    ...
    PrimitiveArray, GenericStringArray, GenericBinaryArray, GenericListArray,
};
...
macro_rules! define_row_builder {
    (
        struct $name:ident<$generic:ident: $($bound:path)+>
        $(where $where_clause:tt)?,
        |$array_param:ident| -> $array_type:ty { $init_expr:expr }
    ) => {
        pub(crate) struct $name<'a, $generic: $($bound)+> 
        $(where $where_clause)?
        {
            array: &'a $array_type,
        }
        
        impl<'a, $generic: $($bound)+> $name<'a, $generic>
        $(where $where_clause)?
        {
            fn new($array_param: &'a dyn Array) -> Self {
                Self {
                    array: $init_expr,
                }
            }
...
define_row_builder!(
    struct PrimitiveArrowToVariantBuilder<T: ArrowPrimitiveType>
    where T::Native: Into<Variant<'a, 'a>>,
    |array| -> PrimitiveArray<T> { array.as_primitive() }
);

// String builder - handles String and LargeString
define_row_builder!(
    struct StringArrowToVariantBuilder<O: OffsetSizeTrait>,
    |array| -> GenericStringArray<O> { array.as_string() }
);

// Binary builder - handles Binary and LargeBinary
define_row_builder!(
    struct BinaryArrowToVariantBuilder<O: OffsetSizeTrait>,
