...
// Builds a BinaryArray of `size` rows over `unique_count` distinct byte strings, 5% nulls.
// Binary and Utf8 both route through pack_byte_to_dictionary, so binary covers both paths.
fn build_binary_array_for_dict_cast(size: usize, unique_count: usize) -> ArrayRef {
