...
fn read_unsigned(data: &[u8], pos: &mut usize, size: u8) -> Result<usize, ArrowError> {
    if *pos + (size as usize - 1) >= data.len() {
        return Err(ArrowError::InvalidArgumentError(
            format!("Unexpected end of data for {} byte unsigned integer", size)
        ));
    }
...
fn decode_value_internal(data: &[u8], pos: &mut usize, keys: &[String]) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data".to_string()));
    }
    ...
    match get_basic_type(header) {
        ...
        VariantBasicType::ShortString => {
            ...
            if *pos + len as usize > data.len() {
                return Err(ArrowError::InvalidArgumentError("Unexpected end of data for short string".to_string()));
            }
        ...
        VariantBasicType::Object => {
            ...
            for i in 0..num_elements {
                ...
                if field_id >= keys.len() {
                    return Err(ArrowError::InvalidArgumentError(format!("Field ID out of range: {}", field_id)));
                }
                ...
                if base_pos + end_offset > data.len() {
                    return Err(ArrowError::SchemaError("Unexpected end of data for object field".to_string()));
                }
        ...
        VariantBasicType::Array => {
            ...
            for i in 0..num_elements {
                ...
                if base_pos + end_offset > data.len() {
                    return Err(ArrowError::SchemaError("Unexpected end of data for array element".to_string()));
                }
...
fn decode_primitive(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for primitive".to_string()));
    }
    ...
    *pos += 1;
    ...
    match type_id {
        ...
        _ => Err(ArrowError::SchemaError(format!("Unknown primitive type ID: {}", type_id)))
    }
}
...
fn decode_short_string(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for short string length".to_string()));
    }
    ...
    *pos += 1;
    ...
    if *pos + len > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for short string content".to_string()));
    }
...
fn decode_int8(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for int8".to_string()));
    }
...
fn decode_int16(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 1 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for int16".to_string()));
    }
...
fn decode_int32(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for int32".to_string()));
    }
...
fn decode_int64(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for int64".to_string()));
    }
...
fn decode_double(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for double".to_string()));
    }
...
fn decode_decimal4(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 4 > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for decimal4".to_string()));
    }
...
fn decode_decimal8(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 8 > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for decimal8".to_string()));
    }
    ...
    *pos += 7;
    ...
    Ok(Value::Number(serde_json::Number::from_f64(value)
...
fn decode_decimal16(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 16 > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for decimal16".to_string()));
    }
...
fn decode_date(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for date".to_string()));
    }
...
fn decode_timestamp(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for timestamp".to_string()));
    }
...
fn decode_timestamp_ntz(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for timestamp_ntz".to_string()));
    }
...
fn decode_float(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for float".to_string()));
    }
...
fn decode_binary(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for binary length".to_string()));
    }
    ...
    *pos += 4;
    ...
    if *pos + len > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for binary content".to_string()));
    }
...
fn decode_long_string(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for string length".to_string()));
    }
    ...
    *pos += 4;
    ...
    // Read the string bytes
    if *pos + len > data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for string content".to_string()));
    }
...
fn decode_time_ntz(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for time_ntz".to_string()));
    }
...
fn decode_timestamp_nanos(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for timestamp_nanos".to_string()));
    }
...
fn decode_timestamp_ntz_nanos(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for timestamp_ntz_nanos".to_string()));
    }
...
fn decode_uuid(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 15 >= data.len() {
        return Err(ArrowError::InvalidArgumentError("Unexpected end of data for uuid".to_string()));
    }
...
pub fn parse_metadata_keys(metadata: &[u8]) -> Result<Vec<String>, ArrowError> {
    ...
    if version != 1 {
        return Err(ArrowError::SchemaError(format!("Unsupported version: {}", version)));
    }
    
    if metadata.len() < 1 + offset_size {
        return Err(ArrowError::SchemaError("Metadata too short for dictionary size".to_string()));
    }
    ...
    if metadata.len() < offset_end {
        return Err(ArrowError::SchemaError("Metadata too short for offsets".to_string()));
    }
...
pub fn is_object(value: &[u8]) -> Result<bool, ArrowError> {
    if value.is_empty() {
        return Err(ArrowError::InvalidArgumentError("Empty value data".to_string()));
    }
...
pub fn get_field_value_range(value: &[u8], metadata: &[u8], key: &str) -> Result<Option<(usize, usize)>, ArrowError> {
    ...
    while low <= high {
        ...
        if pos + id_size as usize <= value.len() {
            ...
            if let Some(field_key) = dict.get_key(id) {
                ...
                return Err(ArrowError::InvalidArgumentError(
                    format!("Field ID {} not found in metadata dictionary", id)
                ));
            }
        } else {
            return Err(ArrowError::InvalidArgumentError(
                format!("Field ID position out of bounds: {} + {}", pos, id_size)
            ));
        }
    }
    ...
    let idx = match field_index {
        Some(idx) => idx,
        None => {
            println!("Field ID {} not found in object fields: {:?}", field_id, found_fields);
            return Ok(None);
        }
    };
    ...
    if field_end > value.len() {
        return Err(ArrowError::InvalidArgumentError(
            format!("Field offset out of bounds: {} > {}", field_end, value.len())
        ));
    }
...
pub fn get_field_value(value: &[u8], metadata: &[u8], key: &str) -> Result<Option<Vec<u8>>, ArrowError> {
...
pub fn get_array_element_range(value: &[u8], index: usize) -> Result<Option<(usize, usize)>, ArrowError> {
    ...
    if elem_end > value.len() {
        return Err(ArrowError::InvalidArgumentError(
            format!("Element offset out of bounds: {} > {}", elem_end, value.len())
        ));
    }
...
pub fn decode_string(value: &[u8]) -> Result<String, ArrowError> {
    if value.is_empty() {
        return Err(ArrowError::InvalidArgumentError("Empty value buffer".to_string()));
    }
    ...
    match get_basic_type(header) {
        VariantBasicType::ShortString => {
            ...
            if value.len() < 1 + len as usize {
                return Err(ArrowError::InvalidArgumentError(
                    format!("Buffer too short for short string: expected {} bytes", 1 + len)
                ));
            }
            ...
            String::from_utf8(string_bytes.to_vec())
                .map_err(|e| ArrowError::InvalidArgumentError(
        ...
        VariantBasicType::Primitive => {
            ...
            match primitive_type {
                VariantPrimitiveType::String => {
                    ...
                    if value.len() < 5 + len {
                        return Err(ArrowError::InvalidArgumentError(
                            format!("Buffer too short for long string: expected {} bytes", 5 + len)
                        ));
                    }
                ...
                _ => Err(ArrowError::InvalidArgumentError(
                    format!("Not a string value, primitive type: {:?}", primitive_type)
                )),
            }
        },
        _ => Err(ArrowError::InvalidArgumentError(
            format!("Not a string value, header: {:#x}", header)
        )),
    }
}
        ...
        VariantBasicType::Primitive => {
            ...
            match primitive_type {
                VariantPrimitiveType::Int8 => {
                    if value.len() < 2 {
                        return Err(ArrowError::InvalidArgumentError("Buffer too short for int8".to_string()));
                    }
                    Ok(value[1] as i8 as i32)
                },
                VariantPrimitiveType::Int16 => {
                    if value.len() < 3 {
                        return Err(ArrowError::InvalidArgumentError("Buffer too short for int16".to_string()));
                    }
                    Ok(i16::from_le_bytes([value[1], value[2]]) as i32)
                },
                VariantPrimitiveType::Int32 => {
                    if value.len() < 5 {
                        return Err(ArrowError::InvalidArgumentError("Buffer too short for int32".to_string()));
                    }
                    Ok(i32::from_le_bytes([value[1], value[2], value[3], value[4]]))
                },
                VariantPrimitiveType::Int64 => {
                    if value.len() < 9 {
                        return Err(ArrowError::InvalidArgumentError("Buffer too short for int64".to_string()));
                    }
                    ...
                    if v > i32::MAX as i64 || v < i32::MIN as i64 {
                        return Err(ArrowError::InvalidArgumentError(
                            format!("i64 value {} is out of range for i32", v)
                        ));
                    }
                    Ok(v as i32)
                },
                _ => Err(ArrowError::InvalidArgumentError(
                    format!("Not an integer value, primitive type: {:?}", primitive_type)
                )),
            }
        },
        _ => Err(ArrowError::InvalidArgumentError(
            format!("Not an integer value, header: {:#x}", header)
        )),
    }
}
    ...
    match get_basic_type(header) {
        VariantBasicType::Primitive => {
            ...
            match primitive_type {
                ...
                _ => Err(ArrowError::InvalidArgumentError(
                    format!("Not a boolean value, primitive type: {:?}", primitive_type)
                )),
            }
        },
        _ => Err(ArrowError::InvalidArgumentError(
            format!("Not a boolean value, header: {:#x}", header)
        )),
    }
}
    ...
    match get_basic_type(header) {
        VariantBasicType::Primitive => {
            ...
            match primitive_type {
                ...
                _ => Err(ArrowError::InvalidArgumentError(
                    format!("Not a double value, primitive type: {:?}", primitive_type)
                )),
            }
        },
        _ => Err(ArrowError::InvalidArgumentError(
            format!("Not a double value, header: {:#x}", header)
        )),
    }
}
