...
fn read_unsigned(data: &[u8], pos: &mut usize, size: u8) -> Result<usize, ArrowError> {
    if *pos + (size as usize - 1) >= data.len() {
        return Err(ArrowError::SchemaError(format!("Unexpected end of data for {} byte unsigned integer", size)));
    }
...
fn decode_value_internal(data: &[u8], pos: &mut usize, keys: &[String]) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data".to_string()));
    }
    ...
    match get_basic_type(header) {
        ...
        VariantBasicType::ShortString => {
            ...
            if *pos + len as usize > data.len() {
                return Err(ArrowError::SchemaError("Unexpected end of data for short string".to_string()));
            }
        ...
        VariantBasicType::Object => {
            ...
            for i in 0..num_elements {
                ...
                if field_id >= keys.len() {
                    return Err(ArrowError::SchemaError(format!("Field ID out of range: {}", field_id)));
                }
...
fn decode_primitive(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for primitive".to_string()));
    }
...
fn decode_short_string(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for short string length".to_string()));
    }
    ...
    *pos += 1;
    ...
    if *pos + len > data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for short string content".to_string()));
    }
...
fn decode_int8(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for int8".to_string()));
    }
...
fn decode_int16(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 1 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for int16".to_string()));
    }
...
fn decode_int32(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for int32".to_string()));
    }
...
fn decode_int64(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for int64".to_string()));
    }
...
fn decode_double(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for double".to_string()));
    }
...
fn decode_decimal4(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 4 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for decimal4".to_string()));
    }
...
fn decode_decimal8(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 8 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for decimal8".to_string()));
    }
...
fn decode_decimal16(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 16 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for decimal16".to_string()));
    }
...
fn decode_date(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for date".to_string()));
    }
...
fn decode_timestamp(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for timestamp".to_string()));
    }
...
fn decode_timestamp_ntz(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for timestamp_ntz".to_string()));
    }
...
fn decode_float(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for float".to_string()));
    }
...
fn decode_binary(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for binary length".to_string()));
    }
    ...
    *pos += 4;
    ...
    if *pos + len > data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for binary content".to_string()));
    }
...
fn decode_long_string(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 3 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for string length".to_string()));
    }
    ...
    *pos += 4;
    ...
    // Read the string bytes
    if *pos + len > data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for string content".to_string()));
    }
...
fn decode_time_ntz(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for time_ntz".to_string()));
    }
...
fn decode_timestamp_nanos(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for timestamp_nanos".to_string()));
    }
...
fn decode_timestamp_ntz_nanos(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 7 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for timestamp_ntz_nanos".to_string()));
    }
...
fn decode_uuid(data: &[u8], pos: &mut usize) -> Result<Value, ArrowError> {
    if *pos + 15 >= data.len() {
        return Err(ArrowError::SchemaError("Unexpected end of data for uuid".to_string()));
    }
...
fn parse_metadata_keys(metadata: &[u8]) -> Result<Vec<String>, ArrowError> {
    if metadata.is_empty() {
        return Err(ArrowError::SchemaError("Empty metadata".to_string()));
    }
    ...
    for i in 0..dictionary_size as usize {
        ...
        if end > metadata.len() {
            return Err(ArrowError::SchemaError("Invalid string offset".to_string()));
        }
...
mod tests {
    ...
    fn encode_and_decode(value: Value) -> Result<Value, ArrowError> {
    ...
    #[test]
    fn test_decode_primitives() -> Result<(), ArrowError> {
        // Test null
    ...
    #[test]
    fn test_decode_array() -> Result<(), ArrowError> {
    ...
    #[test]
    fn test_decode_object() -> Result<(), ArrowError> {
    ...
    #[test]
    fn test_decode_complex() -> Result<(), ArrowError> {
    ...
    #[test]
    fn test_decode_primitive_function() -> Result<(), ArrowError> {
        // Test with null type
        let mut pos = 0;
    ...
    #[test]
    fn test_decode_short_string_function() -> Result<(), ArrowError> {
        let mut pos = 0;
    ...
    #[test]
    fn test_decode_string_function() -> Result<(), ArrowError> {
        let mut pos = 0;
