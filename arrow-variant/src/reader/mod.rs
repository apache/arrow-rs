...
/// ```
pub fn from_json(json_str: &str) -> Result<Variant, ArrowError> {
...
/// ```
pub fn from_json_array(json_strings: &[&str]) -> Result<StructArray, ArrowError> {
...
/// println!("Value length: {}", variant.value().len());
/// ```
pub fn from_json_value(value: &Value) -> Result<Variant, ArrowError> {
...
/// assert_eq!(variant_array.len(), 2);
/// ```
pub fn from_json_value_array(values: &[Value]) -> Result<StructArray, ArrowError> {
    if values.is_empty() {
        return Err(Error::EmptyInput);
    }
