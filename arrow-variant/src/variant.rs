...
impl<'a> Variant<'a> {
    ...
    pub fn as_string(&self) -> Result<String, ArrowError> {
        match self.as_value()? {
            ...
            _ => Err(ArrowError::InvalidArgumentError("Cannot convert value to string".to_string()))
        }
    }
    ...
    pub fn as_i32(&self) -> Result<i32, ArrowError> {
        match self.as_value()? {
            serde_json::Value::Number(n) => {
                ...
                Err(ArrowError::InvalidArgumentError("Number outside i32 range".to_string()))
            },
            _ => Err(ArrowError::InvalidArgumentError("Cannot convert value to i32".to_string()))
        }
    }
    ...
    pub fn as_i64(&self) -> Result<i64, ArrowError> {
        match self.as_value()? {
            serde_json::Value::Number(n) => {
                ...
                Err(ArrowError::InvalidArgumentError("Number cannot be represented as i64".to_string()))
            },
            _ => Err(ArrowError::InvalidArgumentError("Cannot convert value to i64".to_string()))
        }
    }
    ...
    pub fn as_bool(&self) -> Result<bool, ArrowError> {
        match self.as_value()? {
            ...
            serde_json::Value::Number(n) => {
                ...
                Err(ArrowError::InvalidArgumentError("Cannot convert number to bool".to_string()))
            },
            serde_json::Value::String(s) => {
                match s.to_lowercase().as_str() {
                    "true" | "yes" | "1" => Ok(true),
                    "false" | "no" | "0" => Ok(false),
                    _ => Err(ArrowError::InvalidArgumentError("Cannot convert string to bool".to_string()))
                }
            },
            _ => Err(ArrowError::InvalidArgumentError("Cannot convert value to bool".to_string()))
        }
    }
    ...
    pub fn as_f64(&self) -> Result<f64, ArrowError> {
        match self.as_value()? {
            serde_json::Value::Number(n) => {
                ...
                Err(ArrowError::InvalidArgumentError("Number cannot be represented as f64".to_string()))
            },
            serde_json::Value::String(s) => {
                s.parse::<f64>()
                    .map_err(|_| ArrowError::InvalidArgumentError("Cannot parse string as f64".to_string()))
            },
            _ => Err(ArrowError::InvalidArgumentError("Cannot convert value to f64".to_string()))
        }
    }
...
impl<'a> fmt::Display for Variant<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match decoder::format_variant_value(self.value, self.metadata) {
            ...
            Err(_) => write!(f, "Variant(metadata={} bytes, value={} bytes)", 
