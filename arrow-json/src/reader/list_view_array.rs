...
pub struct ListViewArrayDecoder<O> {
    data_type: DataType,
...
impl<O: OffsetSizeTrait> ListViewArrayDecoder<O> {
    pub fn new(
        ctx: &DecoderContext,
        data_type: &DataType,
        is_nullable: bool,
        ...
        let field = match data_type {
            DataType::ListView(f) if !O::IS_LARGE => f,
            DataType::LargeListView(f) if O::IS_LARGE => f,
            _ => unreachable!(),
        };
...
impl<O: OffsetSizeTrait> ArrayDecoder for ListViewArrayDecoder<O> {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayData, ArrowError> {
        let mut child_pos = Vec::with_capacity(pos.len());
        let mut offsets = BufferBuilder::<O>::new(pos.len());
        let mut sizes = BufferBuilder::<O>::new(pos.len());
        ...
        for p in pos {
            ...
            let size = O::from_usize(child_pos.len() - start_count).ok_or_else(|| {
