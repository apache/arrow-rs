...
impl ReaderBuilder {
    ...
    /// present in `schema`. If `struct_mode` is `ListOnly`, then it is required
    /// for all fields of the struct to be in the list, regardless of the value
    /// of `strict_mode`. Without field names, there is no way to determine
...
mod tests {
    ...
    fn test_struct_decoding() {
        ...
        let parse = |s: &str, mode: StructMode| {
            ReaderBuilder::new(schema.clone())
