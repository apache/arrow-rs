...
impl ArrayDecoder for StructArrayDecoder {
    fn decode(&mut self, tape: &Tape<'_>, pos: &[u32]) -> Result<ArrayData, ArrowError> {
        ...
        for (row, p) in pos.iter().enumerate() {
            let end_idx = match (tape.get(*p), nulls.as_mut(), self.struct_parse_mode) {
                (TapeElement::StartObject(end_idx), None, StructParseMode::ObjectOnly) => end_idx,
                (TapeElement::StartObject(end_idx), Some(nulls), StructParseMode::ObjectOnly) => {
                    nulls.append(true);
                    end_idx
                }
                (TapeElement::StartList(end_idx), None, StructParseMode::ListOnly) => end_idx,
                (TapeElement::StartList(end_idx), Some(nulls), StructParseMode::ListOnly) => {
                    nulls.append(true);
                    end_idx
                }
                (TapeElement::Null, Some(nulls), _) => {
                    nulls.append(false);
                    continue;
                }
                (_, _, StructParseMode::ObjectOnly) => return Err(tape.error(*p, "{")),
                (_, _, StructParseMode::ListOnly) => return Err(tape.error(*p, "[")),
            };

            let mut cur_idx = *p + 1;
            if self.struct_parse_mode == StructParseMode::ObjectOnly {
