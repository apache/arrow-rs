...
mod tests {
    ...
    #[test]
    fn test_sparse_mixed_build_cloned() {
        let mut builder = UnionBuilder::new_sparse();
        builder.append::<Int32Type>("a", 1).unwrap();
        builder.append::<Float64Type>("b", 3.0).unwrap();
        builder.append::<Int32Type>("c", 4).unwrap();
        let union_cloned = builder.build_cloned().unwrap();
        builder.append::<Float64Type>("b", 5.0).unwrap();
        builder.append::<Int32Type>("a", 6).unwrap();
        let union = builder.build().unwrap();

        let expected_type_ids_cloned = vec![0_i8, 1, 2];
        let expected_type_ids = vec![0_i8, 1, 2, 1, 0];

        // Check type ids
        assert_eq!(
            Buffer::from_slice_ref(&expected_type_ids_cloned),
            union_cloned.data().buffers()[0]
        );
        assert_eq!(
            Buffer::from_slice_ref(&expected_type_ids),
            union.data().buffers()[0]
        );
        for (i, id) in expected_type_ids_cloned.iter().enumerate() {
            assert_eq!(id, &union_cloned.type_id(i));
        }
        for (i, id) in expected_type_ids.iter().enumerate() {
            assert_eq!(id, &union.type_id(i));
        }
        ...
        // Check offsets, sparse union should only have a single buffer, i.e. no offsets
        assert_eq!(union_cloned.data().buffers().len(), 1);
        assert_eq!(union.data().buffers().len(), 1);

        for i in 0..union_cloned.len() {
            let slot = union_cloned.value(i);
            assert!(!union_cloned.is_null(i));
            match i {
                0 => {
                    let slot = slot.as_any().downcast_ref::<Int32Array>().unwrap();
                    assert_eq!(slot.len(), 1);
