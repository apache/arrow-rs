...
impl VariantIntegrationHarness {
    ...
    fn new() -> Result<Self, Box<dyn Error>> {
        ...
        println!("Loaded {} test cases from {}", test_cases.len(), test_data_dir.display());
    ...
    fn run_all_tests(&self) -> Result<(), Box<dyn Error>> {
        ...
        for test_case in &self.test_cases {
            if !test_case.enabled {
                println!("IGNORED: case-{:03} - {}", test_case.case_number, 
            ...
            match self.run_single_test(test_case) {
                ...
                Err(e) => {
                    println!("FAILED: case-{:03} - {} - Error: {}", test_case.case_number,
                        test_case.test.as_deref().unwrap_or("unknown test"), e);
                    failed += 1;
                }
            }
        }
    ...
    fn run_success_test(&self, test_case: &VariantTestCase) -> Result<(), Box<dyn Error>> {
        ...
        println!("  {} validation passed for case-{:03}", 
                 match test_case.test_category {
        ...
        println!("    Found {} StructArray(s) with variant structure", struct_arrays.len());
    ...
    fn run_error_test(&self, test_case: &VariantTestCase) -> Result<(), Box<dyn Error>> {
        ...
        match self.read_parquet_file(test_case) {
            Ok(struct_arrays) => {
                ...
                println!("    Parquet file read successfully, expecting error during variant processing");
    ...
    fn load_expected_variant_data(&self, test_case: &VariantTestCase) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    fn load_expected_variant_data_by_file(&self, variant_file: &str) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    fn read_parquet_file(&self, test_case: &VariantTestCase) -> Result<Vec<StructArray>, Box<dyn Error>> {
    ...
    fn is_variant_struct_array(&self, struct_array: &StructArray) -> Result<bool, Box<dyn Error>> {
        ...
        let field_names: Vec<&str> = column_names.iter().map(|s| s.as_ref()).collect();
    ...
    fn verify_variant_structure(&self, struct_arrays: &[StructArray]) -> Result<(), Box<dyn Error>> {
        for (i, struct_array) in struct_arrays.iter().enumerate() {
            if !self.is_variant_struct_array(struct_array)? {
                return Err(format!("StructArray {} does not have expected Variant structure", i).into());
            }
            
            println!("    StructArray {} has {} rows and valid Variant structure", i, struct_array.len());
        }
...
fn load_test_cases(test_data_dir: &PathBuf) -> Result<Vec<VariantTestCase>, Box<dyn Error>> {
        ...
        if trimmed.contains("\"case_number\"") {
            ...
            if let Some(case) = current_case.as_mut() {
                if let Some(start_quote) = trimmed.rfind('"') {
                    let before_quote = &trimmed[..start_quote];
                    if let Some(second_quote) = before_quote.rfind('"') {
                        case.variant_description = Some(before_quote[second_quote + 1..].to_string());
                    }
                }
            }
            ...
            if let Some(mut case) = current_case.take() {
                if !case.parquet_file.is_empty() && (case.variant_file.is_some() || case.error_message.is_some()) {
...
fn test_variant_integration_case_001() {
    ...
    let test_case = harness.test_cases.iter()
        .find(|case| case.case_number == 1)
        .expect("case-001 not found");
        
    harness.run_single_test(test_case).expect("case-001 should pass");
}
...
fn test_variant_integration_case_002() {
    ...
    harness.run_single_test(test_case).expect("case-002 should pass");
}
...
fn test_variant_integration_case_004() {
    ...
    harness.run_single_test(test_case).expect("case-004 should pass");
}
...
fn test_variant_integration_case_005() {
    ...
    harness.run_single_test(test_case).expect("case-005 should pass");
}
...
fn test_variant_integration_case_006() {
    ...
    harness.run_single_test(test_case).expect("case-006 should pass");
}
...
fn test_variant_integration_case_007() {
    ...
    harness.run_single_test(test_case).expect("case-007 should pass");
}
...
fn test_variant_integration_case_008() {
    ...
    harness.run_single_test(test_case).expect("case-008 should pass");
}
...
fn test_variant_integration_case_009() {
    ...
    harness.run_single_test(test_case).expect("case-009 should pass");
}
...
fn test_variant_integration_case_010() {
    ...
    harness.run_single_test(test_case).expect("case-010 should pass");
}
...
fn test_variant_integration_error_case_040() {
    ...
    harness.run_single_test(test_case).expect("Error case should be handled gracefully");
}
...
fn test_variant_integration_all_cases() {
    let harness = VariantIntegrationHarness::new().expect("Failed to create test harness");
    harness.run_all_tests().expect("Integration tests should pass");
}
...
fn test_variant_integration_primitives_only() {
    ...
    let primitive_cases: Vec<_> = harness.test_cases.iter()
...
fn test_variant_integration_arrays_only() {
    ...
    let array_cases: Vec<_> = harness.test_cases.iter()
    ...
    for test_case in array_cases {
        println!("Testing case-{:03}: {}", test_case.case_number, 
...
fn test_variant_integration_objects_only() {
    ...
    let object_cases: Vec<_> = harness.test_cases.iter()
...
fn test_variant_integration_error_cases_only() {
    ...
    let error_cases: Vec<_> = harness.test_cases.iter()
    ...
    for test_case in error_cases {
        println!("Testing error case-{:03}: {}", test_case.case_number,
                 test_case.test.as_deref().unwrap_or("unknown"));
        println!("  Expected error: {}", test_case.error_message.as_deref().unwrap_or("none"));
        match harness.run_single_test(test_case) {
...
fn test_variant_structure_validation() {
    ...
    match VariantIntegrationHarness::new() {
        Ok(harness) => {
            println!("Successfully loaded test harness with {} test cases", harness.test_cases.len());
            ...
            for case_number in test_cases_to_validate {
                if let Some(test_case) = harness.test_cases.iter().find(|c| c.case_number == case_number) {
                    println!("\nValidating case-{:03}: {}", case_number, test_case.parquet_file);
                    ...
                    match harness.run_single_test(test_case) {
                        ...
                        Err(e) => {
                            println!("  Structure validation FAILED for case-{:03}: {}", case_number, e);
            ...
            println!("\nValidated {} test case structures successfully", validated_cases);
        }
        Err(e) => {
...
fn test_variant_integration_comprehensive_analysis() {
    ...
    match VariantIntegrationHarness::new() {
        Ok(harness) => {
            ...
            println!("  Primitives: {}", category_counts.get(&TestCategory::Primitives).unwrap_or(&0));
            println!("  Arrays: {}", category_counts.get(&TestCategory::Arrays).unwrap_or(&0));
            println!("  Objects: {}", category_counts.get(&TestCategory::Objects).unwrap_or(&0));
            println!("  Error Handling: {}", category_counts.get(&TestCategory::ErrorHandling).unwrap_or(&0));
            println!("  Schema Validation: {}", category_counts.get(&TestCategory::SchemaValidation).unwrap_or(&0));
            println!("  Complex: {}", category_counts.get(&TestCategory::Complex).unwrap_or(&0));
            ...
            for case_number in test_cases_to_check {
                if let Some(test_case) = harness.test_cases.iter().find(|c| c.case_number == case_number) {
                    println!("Case-{:03} ({:?}): {} -> {}", 
                        ...
                        test_case.variant_file.as_deref().unwrap_or("no variant file"));
                    
                    // Verify files exist
                    let parquet_path = harness.test_data_dir.join(&test_case.parquet_file);
                    assert!(parquet_path.exists(), "Parquet file should exist: {}", parquet_path.display());
                    
                    if let Some(variant_file) = &test_case.variant_file {
                        let variant_path = harness.test_data_dir.join(variant_file);
                        assert!(variant_path.exists(), "Variant file should exist: {}", variant_path.display());
            ...
            for error_case in error_cases.iter().take(3) {
                println!("  Case-{:03}: {} - {}", 
                    ...
                    error_case.error_message.as_deref().unwrap_or("no error message"));
            }
            
            assert!(validated_cases >= 3, "Should validate at least 3 test cases");
            assert!(!harness.test_cases.is_empty(), "Should have loaded test cases");
    ...
    match VariantIntegrationHarness::new() {
        Ok(harness) => {
            ...
            for error_case in &error_cases {
                ...
                assert!(parquet_path.exists(), "Error case parquet file should exist: {}", parquet_path.display());
    ...
    match variant_dir {
        Ok(dir) => {
            ...
            if let Ok(entries) = fs::read_dir(&dir) {
                ...
                assert!(!metadata_files.is_empty(), "Should find at least some metadata files");
...
fn test_load_existing_variant_case(variant_dir: &PathBuf, case_name: &str) -> Result<(), Box<dyn Error>> {
