...
impl BoundingBox {
    ...
    /// Updates the X and Y coordinates of the bounding box with new point.
    ...
    /// Updates the X, Y, and Z coordinates of the bounding box with new point.
    pub fn update_xyz(&mut self, x: f64, y: f64, z: Option<f64>) {
        self.update_xy(x, y);

        if let Some(z) = z {
            self.zmin = Some(self.zmin.unwrap_or(z).min(z));
            self.zmax = Some(self.zmax.unwrap_or(z).max(z));
        }
    }

    /// Updates the X, Y, and M coordinates of the bounding box with new point.
    pub fn update_xym(&mut self, x: f64, y: f64, m: Option<f64>) {
        self.update_xy(x, y);

        if let Some(m) = m {
            self.mmin = Some(self.mmin.unwrap_or(m).min(m));
            self.mmax = Some(self.mmax.unwrap_or(m).max(m));
        }
    }

    /// Updates the X, Y, Z, and M coordinates of the bounding box with new point.
    pub fn update_xyzm(&mut self, x: f64, y: f64, z: Option<f64>, m: Option<f64>) {
        self.update_xy(x, y);
    ...
    /// Merges this bounding box with another, expanding to contain both.
    /// 
    /// This function updates the current bounding box to encompass all coordinates
    /// from both bounding boxes. If any coordinate dimension contains NaN values
    /// in either bounding box, that dimension will be set to NaN in the result.
    pub fn merge(&mut self, other: &Self) {
        // Helper function to merge two coordinate values, handling NaN cases
        #[inline]
        fn merge_coord(min1: f64, max1: f64, min2: f64, max2: f64) -> (f64, f64) {
            if min1.is_nan() || max1.is_nan() || min2.is_nan() || max2.is_nan() {
                (f64::NAN, f64::NAN)
            } else {
        ...
        // Helper function to merge two Z and M coordinate values, handling NaN and None cases
        #[inline]
        fn merge_zm_coords(min1: Option<f64>, max1: Option<f64>, min2: Option<f64>, max2: Option<f64>) -> (Option<f64>, Option<f64>) {
            return match (min1, max1, min2, max2) {
                (Some(min1), Some(max1), Some(min2), Some(max2)) => {
                    let (min_merged, max_merged) = merge_coord(min1, max1, min2, max2);
                    (Some(min_merged), Some(max_merged))
                }
                (Some(min1), Some(max1), None, None) => (Some(min1), Some(max1)),
                (None, None, Some(min2), Some(max2)) => (Some(min2), Some(max2)),
                _ => (None, None),
            };
        }

        // Merge X and Y coordinates
        let (xmin, xmax) = merge_coord(self.xmin, self.xmax, other.xmin, other.xmax);
        let (ymin, ymax) = merge_coord(self.ymin, self.ymax, other.ymin, other.ymax);

        // Merge Z and M coordinates
        let (zmin, zmax) = merge_zm_coords(self.zmin, self.zmax, other.zmin, other.zmax);
        let (mmin, mmax) = merge_zm_coords(self.mmin, self.mmax, other.mmin, other.mmax);
