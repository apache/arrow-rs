    ...
    fn skip_values(&mut self, num_values: usize) -> Result<usize> {
        match self.decoder.as_mut().expect("decoder not set in DictionaryDecoder.") {
            ...
            MaybeDictionaryDecoder::Dict {
                ...
                let decoder_skip_value = decoder.skip(to_skip).unwrap();
                *max_remaining_values -= decoder_skip_value;
                Ok(decoder_skip_value)
            }
        }
    }
}
...
mod tests {
    ...
    fn test_dictionary_preservation_skip() {
        ...
        let valid_buffer = Buffer::from_iter(valid.iter().cloned());
        output.pad_nulls(6, 3, 8, valid_buffer.as_slice());
    ...
    #[test]
    fn test_dictionary_fallback_skip() {
        let data_type = utf8_dictionary();
        let mut data = vec!["hello", "world", "a", "b"];
        ...
        for (encoding, page) in pages {
            decoder.set_data(encoding, page, 4, Some(4)).unwrap();
            assert_eq!(decoder.skip_values(1).unwrap(), 1);
            assert_eq!(decoder.read(&mut output, 0..1024).unwrap(), 3);
        }
        let array = output.into_array(None, &data_type).unwrap();
        assert_eq!(array.data_type(), &data_type);
