...
use bytes::Bytes;
use row_group::RowGroupReaderBuilder;
use std::ops::Range;
use std::sync::Arc;
...
impl ParquetPushDecoderBuilder {
    ...
    pub fn build(self) -> Result<ParquetPushDecoder, ParquetError> {
        ...
        let rg_reader_builder = RowGroupReaderBuilder::new(
            file_len,
            parquet_metadata,
            row_groups,
            projection,
        ...
        Ok(ParquetPushDecoder {
            state: ParquetDecoderState::ReadingRowGroup { rg_reader_builder },
        })
    }
}
    ...
    pub fn push_data(
        ...
        match self {
            ParquetDecoderState::ReadingRowGroup {
                ...
                rg_reader_builder.push_data(ranges, data);
    ...
    fn buffered_bytes(&self) -> u64 {
        match self {
            ParquetDecoderState::ReadingRowGroup { rg_reader_builder } => {
                rg_reader_builder.buffered_bytes()
            }
            ParquetDecoderState::DecodingRowGroup {
