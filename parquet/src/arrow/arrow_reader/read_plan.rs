...
impl ReadPlanBuilder {
    /// Create a ReadPlanBuilder with the given batch size and selection
    ///
    /// `selection`: Optional pre-existing selection.
    ...
    /// Returns the currenly cached predicate result, if any
    ...
    /// Returns the number of rows selected, or None for no selection
    ///
    /// Note this does not account for any offset or limit that may
    /// set on this builder
    ...
    /// Finalize the read plan for running
...
/// accounting for any initial [`RowSelection`] and [`RowFilter`].
///
