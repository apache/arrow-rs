...
mod tests {
    ...
    fn test_column_writer_uint32_converted_type_min_max() {
        ...
        let props = Arc::new(WriterProperties::builder().build());
        let mut writer =
            get_test_decimals_column_writer::<Int32Type>(page_writer, 0, 0, props);
        writer.write_batch(&[-2, -1, 0, 1, 2], None, None).unwrap();
        let (_bytes_written, _rows_written, metadata) = writer.close().unwrap();
        if let Some(stats) = metadata.statistics() {
            assert!(stats.has_min_max_set());
            if let Statistics::Int32(stats) = stats {
    ...
    fn test_compare_greater_byte_array_decimals() {
        assert_eq!(compare_greater_byte_array_decimals(&[], &[],), false,);
        assert_eq!(compare_greater_byte_array_decimals(&[1u8,], &[],), true,);
        assert_eq!(compare_greater_byte_array_decimals(&[], &[1u8,],), false,);
        assert_eq!(compare_greater_byte_array_decimals(&[1u8,], &[0u8,],), true,);
        assert_eq!(
            compare_greater_byte_array_decimals(&[1u8,], &[1u8,],),
            false,
        );
        assert_eq!(
            compare_greater_byte_array_decimals(&[1u8, 0u8,], &[0u8,],),
            true,
        );
        assert_eq!(
            compare_greater_byte_array_decimals(&[0u8, 1u8,], &[1u8, 0u8,],),
            false,
        );
        assert_eq!(
            compare_greater_byte_array_decimals(&[255u8, 35u8, 0u8, 0u8,], &[0u8,],),
            false,
        );
        assert_eq!(
            compare_greater_byte_array_decimals(&[0u8,], &[255u8, 35u8, 0u8, 0u8,],),
            true,
        );
    }
