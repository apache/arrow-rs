...
impl<R: 'static + ChunkReader> RowGroupReader for SerializedRowGroupReader<'_, R> {
    ...
    fn get_column_page_reader(&self, i: usize) -> Result<Box<dyn PageReader>> {
        ...
        let page_locations = self
            .offset_index
            .and_then(|x| x[i].as_ref().map(|idx| idx.page_locations.clone()));
...
mod tests {
    ...
    fn test_page_index_reader() {
        ...
        let page_offset = &offset_index[0]
            .as_ref()
    ...
    fn test_page_index_reader_all_type() {
        ...
        let column_index = metadata
            .column_index()
            .expect("column index should be present");
        let row_group_offset_indexes = &metadata
            .offset_index()
            .expect("offset index should be present")[0];
        ...
        if let ColumnIndexMetaData::INT32(index) = column_index[0][2].as_ref().unwrap() {
            ...
            assert_eq!(
                row_group_offset_indexes[2]
                    .as_ref()
                    .expect("offset index should be present for column 2")
                    .page_locations
                    .len(),
                325
            );
