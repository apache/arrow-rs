...
mod async_tests {
    ...
    /// This test verifies that when page index data is corrupted/truncated,
    /// the reader returns an error rather than panicking with an assertion failure
    #[tokio::test]
    async fn test_corrupted_parquet_file_returns_error() {
        // Create a normal parquet file with page indexes
        let file = write_parquet_file(false).unwrap();
        let mut file_reader = file.reopen().unwrap();
        let file_len = file_reader.len();

        // Read the full file
        let mut full_data = Vec::new();
        file_reader.read_to_end(&mut full_data).unwrap();
        ...
        let f = MetadataFetchFn(&mut fetch);
        metadata_reader.try_load(f, file_len).await.unwrap();
        ...
        match needs_data {
            NeedsIndexData::No(_) => {
                // If no index data needed, skip test
                return;
            }
            NeedsIndexData::Yes(_) => {
                // Continue with test
            }
        };

        // Now create a corrupted fetcher that returns insufficient data for the page index
        // This simulates the scenario from the issue where the file is truncated
        let mut corrupted_fetch = |range: Range<u64>| {
            // Return less data than requested to trigger the error
            let requested_len = (range.end - range.start) as usize;
            let bad_data = Bytes::from(vec![0u8; requested_len / 2]); // Return half the data
            futures::future::ready(Ok::<_, ParquetError>(bad_data))
        };

        // Try to load metadata with corrupted data
        let f2 = MetadataFetchFn(&mut corrupted_fetch);
        let result = ParquetMetaDataReader::new()
            .with_page_index_policy(PageIndexPolicy::Required)
            .load_and_finish(f2, file_len)
            .await;

        // Should return an error (either our new error or a parse error), not panic
        assert!(
            result.is_err(),
            "Expected error when reading corrupted parquet file, but got Ok"
        );

        let err_msg = result.unwrap_err().to_string();
        // The error should not be a panic assertion failure
        assert!(
            !err_msg.contains("assertion failed"),
            "Got assertion panic instead of error: {}",
            err_msg
        );
    }
}
