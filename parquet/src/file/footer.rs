...
pub fn parse_metadata2<R: ChunkReader>(chunk_reader: &R) -> Result<ParquetMetaData> {
...
pub fn decode_metadata2(buf: &[u8]) -> Result<ParquetMetaData> {
...
pub fn decode_footer2(slice: &[u8; FOOTER_SIZE]) -> Result<usize> {
    // check this is indeed a parquet file
...
mod tests {
    ...
    fn test_parse_metadata_size_smaller_than_footer() {
        ...
        let reader_result = parse_metadata2(&test_file);
        assert_eq!(
    ...
    fn test_parse_metadata_corrupt_footer() {
        ...
        let reader_result = parse_metadata2(&data);
        assert_eq!(
            reader_result.unwrap_err().to_string(),
