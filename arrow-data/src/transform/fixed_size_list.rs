...
use super::{build_extend_dictionary, build_extend_nulls, build_extend_view, Capacities, Extend, ExtendNullBits, ExtendNulls, _MutableArrayData, preallocate_offset_and_binary_buffer, SpecializedMutableArrayData};
...
/// ```
pub struct FixedSizeListMutableArrayData<'a> {
...
impl<'a> std::fmt::Debug for FixedSizeListMutableArrayData<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        // ignores the closures.
        f.debug_struct("FixedSizeListMutableArrayData")
            .field("data", &self.data)
            .finish()
    }
}

impl<'a> FixedSizeListMutableArrayData<'a> {
    ...
    fn extend_values(&mut self, array_index: usize, start: usize, len: usize) {
        ...
        let size = match self.arrays[array_index].data_type() {
            DataType::FixedSizeList(_, i) => *i as usize,
            _ => unreachable!(),
        };
        ...
        self.data
            .child_data
            .iter_mut()
            .for_each(|child| child.extend(array_index, start * size, (start + len) * size))
    }
    ...
    pub fn with_capacities(
        ...
        let [buffer1, buffer2] = match &capacities {
            ...
            _ => panic!("Capacities: {capacities:?} not supported"),
        };
        ...
        let child_data = match data_type {
            DataType::FixedSizeList(_, size) => {
                ...
                vec![crate::transform::MutableArrayData::with_capacities(
...
impl<'a> SpecializedMutableArrayData<'a> for FixedSizeListMutableArrayData<'a> {
    ...
    fn new(arrays: Vec<&'a ArrayData>, use_nulls: bool, capacity: usize) -> Self {
        Self::with_capacities(arrays, use_nulls, crate::transform::Capacities::Array(capacity))
    }
