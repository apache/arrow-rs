...
mod tests {
    ...
    const YEAR_2000_DAYS: i32 = 10957;
    const YEAR_2000_MILLIS: i64 = 946684800000;
    ...
    fn test_date32_to_naive_date_opt_boundary_values() {
        ...
        // Date32Type::to_naive_date_opt has boundaries determined by NaiveDate's supported range.
    ...
    fn test_date32_add_year_months_opt_boundary_values() {
        ...
        assert!(
            Date32Type::add_year_months_opt(YEAR_2000_DAYS, i32::MAX).is_none(),
            "Adding i32::MAX months should fail"
        );
        assert!(
            Date32Type::add_year_months_opt(YEAR_2000_DAYS, i32::MIN).is_none(),
    ...
    fn test_date32_add_day_time_opt_boundary_values() {
        ...
        assert!(
            Date32Type::add_day_time_opt(near_max_days, interval_30_days).is_some(),
        ...
        assert!(
            Date32Type::add_day_time_opt(near_max_days, interval_large_days).is_none(),
        ...
        assert!(
            Date32Type::add_day_time_opt(near_min_days, interval_minus_30_days).is_some(),
        ...
        assert!(
            Date32Type::add_day_time_opt(near_min_days, interval_minus_large_days).is_none(),
        ...
        assert!(
            Date32Type::add_day_time_opt(YEAR_2000_DAYS, interval_1000_days).is_some(),
        ...
        assert!(
            Date32Type::add_day_time_opt(i32::MIN, IntervalDayTime::new(-1, 0)).is_none(),
        ...
        assert!(
            Date32Type::add_day_time_opt(YEAR_2000_DAYS, large_ms_interval).is_some(),
    ...
    fn test_date32_add_month_day_nano_opt_boundary_values() {
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(near_max_days, interval_safe).is_some(),
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(YEAR_2000_DAYS, zero_interval).is_some(),
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(near_min_days, interval_safe_neg).is_some(),
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(YEAR_2000_DAYS, interval_normal).is_some(),
            "Adding 2 months, 10 days, and nanos to year 2000 should succeed"
        );

        // Test with extreme input values that would cause overflow
        assert!(
            Date32Type::add_month_day_nano_opt(i32::MAX, IntervalMonthDayNano::new(1, 0, 0))
                .is_none(),
            "Adding interval to i32::MAX should fail"
        );
        assert!(
            Date32Type::add_month_day_nano_opt(i32::MIN, IntervalMonthDayNano::new(-1, 0, 0))
                .is_none(),
            "Subtracting interval from i32::MIN should fail"
        );
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(YEAR_2000_DAYS, nano_interval).is_some(),
        ...
        assert!(
            Date32Type::add_month_day_nano_opt(YEAR_2000_DAYS, large_nano_interval).is_some(),
    ...
    fn test_date32_subtract_day_time_opt_boundary_values() {
        ...
        let interval_30_days = IntervalDayTime::new(30, 0);
        assert!(
            Date32Type::subtract_day_time_opt(near_min_days, interval_30_days).is_some(),
            "Subtracting 30 days from large negative year should succeed"
        );
        ...
        let interval_large_days = IntervalDayTime::new(100000000, 0);
        assert!(
            Date32Type::subtract_day_time_opt(near_min_days, interval_large_days).is_none(),
            "Subtracting 100M days from large negative year should fail"
        );
        ...
        let interval_minus_30_days = IntervalDayTime::new(-30, 0);
        assert!(
            Date32Type::subtract_day_time_opt(near_max_days, interval_minus_30_days).is_some(),
            "Adding 30 days to large year should succeed"
        );
        ...
        let interval_minus_large_days = IntervalDayTime::new(-100000000, 0);
        assert!(
            Date32Type::subtract_day_time_opt(near_max_days, interval_minus_large_days).is_none(),
            "Adding 100M days to large year should fail"
        );
        ...
        let interval_1000_days = IntervalDayTime::new(1000, 12345);
        assert!(
            Date32Type::subtract_day_time_opt(YEAR_2000_DAYS, interval_1000_days).is_some(),
        ...
        let interval_one_day = IntervalDayTime::new(1, 0);
        assert!(
            Date32Type::subtract_day_time_opt(i32::MIN, interval_one_day).is_none(),
            "Subtracting interval from i32::MIN should fail"
        );
        assert!(
            Date32Type::subtract_day_time_opt(i32::MAX, IntervalDayTime::new(-1, 0)).is_none(),
            "Adding interval to i32::MAX should fail"
        );
        ...
        let large_ms_interval = IntervalDayTime::new(0, i32::MAX);
        assert!(
            Date32Type::subtract_day_time_opt(YEAR_2000_DAYS, large_ms_interval).is_some(),
        ...
        assert!(
            Date32Type::subtract_day_time_opt(YEAR_2000_DAYS, zero_interval).is_some(),
    ...
    fn test_date32_subtract_month_day_nano_opt_boundary_values() {
        ...
        let interval_safe = IntervalMonthDayNano::new(1, 30, 0);
        assert!(
            Date32Type::subtract_month_day_nano_opt(near_min_days, interval_safe).is_some(),
            "Subtracting 1 month 30 days from large negative year should succeed"
        );
        ...
        let zero_interval = IntervalMonthDayNano::new(0, 0, 0);
        assert!(
            Date32Type::subtract_month_day_nano_opt(YEAR_2000_DAYS, zero_interval).is_some(),
            "Subtracting zero interval should always succeed for valid dates"
        );
        ...
        let interval_safe_neg = IntervalMonthDayNano::new(-1, -30, 0);
        assert!(
            Date32Type::subtract_month_day_nano_opt(near_max_days, interval_safe_neg).is_some(),
            "Adding 1 month 30 days to large year should succeed"
        );

        let interval_normal = IntervalMonthDayNano::new(2, 10, 123_456_789_000);
        assert!(
            Date32Type::subtract_month_day_nano_opt(YEAR_2000_DAYS, interval_normal).is_some(),
            "Subtracting 2 months, 10 days, and nanos from year 2000 should succeed"
        );

        // Test with extreme input values that would cause overflow
        assert!(
            Date32Type::subtract_month_day_nano_opt(i32::MIN, IntervalMonthDayNano::new(1, 0, 0))
                .is_none(),
            "Subtracting interval from i32::MIN should fail"
        );
        assert!(
            Date32Type::subtract_month_day_nano_opt(i32::MAX, IntervalMonthDayNano::new(-1, 0, 0))
                .is_none(),
            "Adding interval to i32::MAX should fail"
        );
        ...
        let nano_interval = IntervalMonthDayNano::new(0, 0, 999_999_999);
        assert!(
            Date32Type::subtract_month_day_nano_opt(YEAR_2000_DAYS, nano_interval).is_some(),
        ...
        let large_nano_interval = IntervalMonthDayNano::new(0, 0, 86_400_000_000_000); // 1 day in nanos
        assert!(
            Date32Type::subtract_month_day_nano_opt(YEAR_2000_DAYS, large_nano_interval).is_some(),
    ...
    fn test_date64_add_year_months_opt_boundary_values() {
        ...
        // Test with extreme input values that would cause overflow
        assert!(
            Date64Type::add_year_months_opt(YEAR_2000_MILLIS, i32::MAX).is_none(),
            "Adding i32::MAX months should fail"
        );
        assert!(
            Date64Type::add_year_months_opt(YEAR_2000_MILLIS, i32::MIN).is_none(),
            "Subtracting i32::MIN months should fail"
        );
        assert!(
